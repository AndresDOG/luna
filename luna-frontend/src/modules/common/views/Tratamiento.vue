<template>
    <v-list>
        <v-list-item>
            ...
            <v-list-item-title><h4 class="font-weight-bold my-0">Tratamiento</h4></v-list-item-title>
            <v-list-item-subtitle class="text-caption grey--text">Contacto>Parametros>Tratamiento</v-list-item-subtitle>
        </v-list-item>
    </v-list>
    <div class="my-3">
        <v-row class="px-lg-5">
            <v-col cols="12">
                ...
                <v-card outlined>
                    <v-toolbar
                        ...
                        <v-toolbar-title><h5 class="font-weight-normal my-0">Tratamiento</h5></v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <template v-slot:item.idaccion="{ item }">
                            <v-tooltip top>
                                <template v-slot:activator="{ props }">
                                  <v-icon v-if="access_etra"  v-bind="props" density="compact" color="blue-grey" @click="ModEdit(item)">mdi-pencil</v-icon>
                                </template>
                                <span>Editar</span>
                            </v-tooltip>
                            ...
                            <v-tooltip top>
                                <template v-slot:activator="{ props }">
                                  <v-icon v-if="access_dtra"  v-bind="props" density="compact" color="pink" @click="eliminar(item.tra_id)">mdi-trash-can</v-icon>
                                </template>
                                <span>Eliminar</span>
                            </v-tooltip>
                        </template>
    ...
    <v-dialog v-model="New" max-width="400px">
        <v-card>
            <v-toolbar
                ...
                <v-toolbar-title><h5 class="font-weight-normal my-0">Crear Tratamiento</h5></v-toolbar-title>
                <v-spacer></v-spacer>
    ...
    <v-dialog v-model="Edit" max-width="400px">
        <v-card>
            <v-toolbar
                ...
                <v-toolbar-title><h5 class="font-weight-normal my-0">Modificar Tratamiento</h5></v-toolbar-title>
                <v-spacer></v-spacer>
...
const access_ctra = ref<boolean>(false);
const access_etra = ref<boolean>(false);
const access_dtra = ref<boolean>(false);
...
const ModEdit = (item:any) =>
{
    ...
    data.value.id = item.tra_id
    data.value.nombre = item.tra_nombre
...
const crear = async () =>
{
    ...
    if(isValid.valid)
    {
        ...
        await authApi.post('api/base/tratamiento',params).then(res =>
        {
            if (res.data === 0)
            {
...
const modificar = async () =>
{
    ...
    if(isValid.valid)
    {
        ...
        await authApi.put(`api/base/tratamiento/${data.value.id}`,params).then(res =>
        {
            if (res.data === 0)
            {
...
const eliminar = async (id:number) =>
{
    Swal.fire({
        ...
        text: 'Confirma eliminar registro de tratamiento?',
    ...
    }).then((result) =>
    {
        if(result.value)
        {
            authApi.delete(`api/base/tratamiento/${id}`).then(res =>
            {
                if (res.data === 0)
                {
...
{
    const componente13 = authStore.permisos.find((p:any) => p.per_id_componente === 13);
    if(componente13)
    {
        const rs =  JSON.parse(componente13.per_acceso);

        access_ctra.value = rs.Crear_Tratamiento === "1" ? true : false;
        access_etra.value= rs.Editar_Tratamiento === "1" ? true : false;
        access_dtra.value = rs.Eliminar_Tratamiento === "1" ? true : false;
    }
};
